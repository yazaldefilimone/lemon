...
1.  Safe by Default: It guarantees memory safety at compile time, preventing whole classes of bugs like _use-after-free_ and _double-free_.
2.  Ergonomic: It removes the need for explicit lifetime annotations in most situations, making code cleaner and the learning curve smoother.
3.  Performant: The ownership semantics allow for compiler optimizations that reduce the number of memory accesses, simplifying verification conditions and bringing code performance close to languages like C.

NOTE: this project is a WORK IN PROGRESS. You're here before the launch. The features described below are in development. Lemon will launch when everything described in this README is implemented.
...
at the IR level, lemon uses a borrow stack to manage memory access, an approach inspired by the OSEA IR. Access to a memory address is only allowed if the reference's tag is at the top of the stack.
...
- Lemon vs. C/C++
  - Guaranteed memory safety at compile time. It eliminates memory leaks, dangling pointers, and data corruption that are common in C/C++ projects.

- Lemon vs. Rust
  - Lemon's ownership cache model is an alternative to Rust's complex lifetime system and borrow checker.
  - The goal is to make 95% of systems programming use cases simpler to write without sacrificing safety. The "fight with the borrow checker" is significantly reduced.
  - The research-backed approach focuses on simplifying the verification logic, which leads to a more straightforward development experience.

## Conclusion

Lemon represents a new perspective on systems programming. By integrating formal verification concepts like pointers with caches and prophecy variables directly into the language's semantics, Lemon aims to find the ideal balance between low-level performance, proven memory safety, and a productive development experience.
